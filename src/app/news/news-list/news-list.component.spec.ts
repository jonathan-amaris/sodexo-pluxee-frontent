...
describe('NewsListComponent', () => {
  ...
  let newsServiceAddToFavoritesSpy: jasmine.Spy;
  ...
  beforeEach(async () => {
    ...
    newsServiceGetNewsSpy = spyOn(newsService, 'getNews').and.returnValue(of(stubValue));
    newsServiceAddToFavoritesSpy = spyOn(newsService, 'addNewsToFavorite').and.returnValue(of(stubValue.results[0]));
  });
  ...
  it('Should called addNewsToFavorite when FavoritesButton is pressed', () => {
    ...
    expect(newsServiceAddToFavoritesSpy).toHaveBeenCalled();
  });
